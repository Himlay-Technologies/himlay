...
import { CheckCircle2, Code, Sparkles } from "lucide-react";
    ...
    <div className="flex flex-col min-h-screen pt-20">
      <section className="py-20 md:py-32 bg-muted/30 relative overflow-hidden">
        <div className="container max-w-screen-xl mx-auto px-4 text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-6">SaaS & Learning Platforms</h1>
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-10">
            Build scalable subscription-based software and comprehensive learning management systems (LMS) designed for global reach.
          </p>
        </div>
      </section>
      ...
      <section className="py-24">
        <div className="container max-w-screen-xl mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold mb-4">Choose Your SaaS/LMS Package</h2>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Multi-tenant architectures built to generate recurring revenue.</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} viewport={{ once: true }} className="glass-card rounded-2xl p-8 border border-border flex flex-col">
              ...
              <Link to="/checkout?plan=starter_saas" className="block w-full py-3 px-4 rounded-md border border-primary text-primary font-medium text-center hover:bg-primary/5 transition-colors">Select Starter</Link>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }} viewport={{ once: true }} className="glass-card rounded-2xl p-8 border-2 border-primary relative shadow-lg scale-100 lg:scale-105 z-10 bg-card flex flex-col">
              ...
              <Link to="/checkout?plan=growth_saas" className="block w-full py-3 px-4 rounded-md bg-primary text-primary-foreground font-medium text-center hover:bg-primary/90 transition-colors shadow-sm">Scale with Growth</Link>
            </motion.div>
